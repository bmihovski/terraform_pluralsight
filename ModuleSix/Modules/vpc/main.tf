...
resource "aws_subnet" "private" {
  ...
  count             = "${length(var.azs)}"
  tags              = "${merge(var.tags, var.private_subnet_tags, map("Name", format("%s-subnet-private", count.index)))}"
}
...
resource "aws_subnet" "public" {
  ...
  availability_zone = "${element(var.azs, count.index)}"
  count             = "${length(var.azs)}"
  tags              = "${merge(var.tags, var.private_subnet_tags, map("Name", format("%s-subnet-prublic", count.index)))}"
