...
/var/log/nginx/*log {
    ...
    postrotate
      ...
      /usr/local/bin/s3cmd sync /var/log/nginx/access.log-* s3://${module.bucket.bucket}/$INSTANCE_ID/nginx/
      /usr/local/bin/s3cmd sync /var/log/nginx/error.log-* s3://${module.bucket.bucket}/$INSTANCE_ID/nginx/
    endscript
}
...
EOF
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "s3cmd get s3://${module.bucket.bucket}/website/index.html .",
      "s3cmd get s3://${module.bucket.bucket}/website/Globo_logo_Vert.png .",
